...
import BehavioralPatternRow from "@/components/ftc/patterns/BehavioralPatternRow";
...
export default function BehavioralPatternList({ patterns }: Props) {
  const [expandedPatternId, setExpandedPatternId] = useState<string | null>(
    null
  );
  const [searchQuery, setSearchQuery] = useState("");
  const [topicFilter, setTopicFilter] = useState<string>("");
  const [sortBy, setSortBy] = useState<SortBy>("cases");
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-3 font-garamond">
        {filtered.length} behavioral pattern{filtered.length !== 1 ? "s" : ""} found
      </p>
