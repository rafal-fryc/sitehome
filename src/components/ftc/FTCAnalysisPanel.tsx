...
export default function FTCAnalysisPanel({ data, mode, groupKey }: Props) {
  ...
  return (
    <Card className="p-6 bg-gradient-to-br from-card to-accent/30 border-0 shadow-soft">
      ...
      {group.top_categories && group.top_categories.length > 0 && (
        <div className="mb-3">
          <p className="text-sm font-medium text-foreground mb-1">Top Categories:</p>
      ...
      {group.top_companies && group.top_companies.length > 0 && (
        <div>
          <p className="text-sm font-medium text-foreground mb-1">Notable Companies:</p>
          <div className="flex flex-wrap gap-1.5">
