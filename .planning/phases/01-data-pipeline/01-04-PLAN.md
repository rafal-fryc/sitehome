...
must_haves:
  truths:
    ...
    - "A sample of 5 manually inspected cases have correct statutory topic and industry sector tags"
...
<objective>
Run the classification script against all 293 FTC source files, then run `npm run build:data` to produce the sharded provision files and enhanced ftc-cases.json. Verify the output distribution is plausible and spot-check a sample of cases.

Purpose: This is the execution and validation step. The scripts are written — now we run them and confirm the data pipeline produces correct, usable output. This plan also formally acknowledges that PIPE-06 (ftc-patterns.json) is deferred to Phase 5 per CONTEXT.md.
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 2: Human spot-check classification quality and distribution</name>
  <what-built>
  ...
  <how-to-verify>
    ...
    2. Spot-check 5 source files you know well:
       - A COPPA case (children's app enforcement) — should have "COPPA" in statutory_topics
       - A credit reporting case — should have "FCRA" in statutory_topics and "Financial Services" in industry_sectors
       - A data security case — should have "Data Security" in practice_areas
       - A telemarketing case — should have "TSR" or "TCPA" in statutory_topics
       - A general Section 5 deceptive practices case — should have "Section 5 Only" in statutory_topics

    3. CRITICAL: Check if "Privacy" is over-represented. If the Privacy practice area shard contains >50% of all provisions across all shards, the keyword matching is too broad. Note this for correction.
  ...
  <resume-signal>
    Type "approved" if classification quality is acceptable for Phase 2 (UI) work.
    Type "fix: [description]" if specific issues need correction before proceeding (e.g., "fix: Privacy is 70% of all provisions — reclassify").
    Type "partial: [description]" if acceptable overall but with noted concerns to track.
  </resume-signal>
</task>
...
- Human spot-check on 5+ cases confirms plausible classification
